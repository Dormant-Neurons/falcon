...
from our_selector_pseudo_loss import LocalPseudoLossSelector
...
def build_buffer(X_train_final, y_train_final, y_train_binary_final, 
                 total_malware_target=525, total_benign_target=0):
    family_indices = defaultdict(list)
    benign_indices = []
    
    for i in range(len(y_train_binary_final)):
        if y_train_binary_final[i] == 0:
            benign_indices.append(i)
        else:
            family_indices[y_train_final[i]].append(i)
    
    families = list(family_indices.keys())
    family_pointers = {f: 0 for f in families}
    families_exhausted = set()
    selected_indices = []
    
    # Malware
    malware_selected = 0
    while malware_selected < total_malware_target:
        added_this_round = 0
        for family in families:
            if family in families_exhausted:
                continue
            if malware_selected >= total_malware_target:
                break
            ptr = family_pointers[family]
            indices = family_indices[family]
            if ptr < len(indices):
                selected_indices.append(indices[ptr])
                family_pointers[family] += 1
                malware_selected += 1
                added_this_round += 1
            else:
                families_exhausted.add(family)
        if added_this_round == 0:
            logging.warning(f"Malware exhausted at {malware_selected}/{total_malware_target}")
            break

    # Benign
    if total_benign_target > 0:
        selected_indices.extend(benign_indices[:total_benign_target])

    retrain_X = np.array([X_train_final[i] for i in selected_indices])
    retrain_y = np.array([y_train_final[i] for i in selected_indices])
    retrain_y_binary = np.array([y_train_binary_final[i] for i in selected_indices])
    
    logging.info(f"Benign: {sum(1 for b in retrain_y_binary if b == 0)}")
    logging.info(f"Malware: {sum(1 for b in retrain_y_binary if b == 1)}")
    logging.info(f"Total: {len(retrain_y_binary)}")
    
    from collections import Counter
    malware_dist = Counter(retrain_y[retrain_y_binary == 1])
    if malware_dist:
        logging.info(f"Family with maximum number of samples: {malware_dist.most_common(1)}")
        logging.info(f"Family with minimum number of samples: {malware_dist.most_common()[-1]}")
    
    return retrain_X, retrain_y, retrain_y_binary
...
def main():
    ...
    malware_X_buffer, malware_y_buffer, malware_y_binary_buffer = build_buffer(X_train_final, y_train_final, y_train_binary_final)
    ...
    if args.encoder != None and args.encoder.startswith('simple-enc-mlp') == True:
        ...
        fully_train_family = all_train_family
    ...
    sample_out.write('date\tCount\tIndex\tGlobalIndex\tTrue\tPred\tFamily\tScore\n')    
    sample_out.flush()
    ...
    test_unc_out = open(args.result.split('.csv')[0] + '_test_unc.csv', 'w')
    test_unc_out.write('date\tIndex\tGlobalIndex\tTrueBinary\tTrueLabel\tPred\tFamily\tUncScore\tPredScore\n')    
    test_unc_out.flush()
    ...
    X_train_buffer = np.empty((0,X_train.shape[1]))
    y_train_buffer = np.empty((0))
    y_train_binary_buffer = np.empty((0))
    
    all_families_seen_so_far = set(np.unique(all_train_family))
    new_fam_data_out = open(args.result.split('.csv')[0]+'_new_fam_in_data.csv', 'w')
    new_fam_data_out.write('date\tNew_fam_in_data_cnt\tNew_fam_in_data\n')
    new_fam_data_out.flush()
    al_step_counter = 0
    EMBEDDING_PLOT_EVERY_N_STEPS = 10
    overall_peak_gpu_mem = 0.0
    while cur_month <= end:
        al_step_counter += 1
        """
        ...
        if args.data.startswith('tesseract'):
            ...
            month_families = set(np.unique(all_test_family))
            new_families_in_data = month_families - all_families_seen_so_far
            new_fam_in_data_cnt = len(new_families_in_data)
            new_families_in_data_str = ",".join(sorted(new_families_in_data))

            new_fam_data_out.write('%s\t%d\t%s\n' % (cur_month_str, new_fam_in_data_cnt, new_families_in_data_str))
            new_fam_data_out.flush()
            
            all_families_seen_so_far.update(month_families)
        ...
        if args.accumulate_data == True:
            y_test_pred, neg_by_fam, family_to_idx = eval_classifier(args, classifier, cur_month_str, X_test_accum_feat, y_test_binary_accum, all_test_family_accum, train_families, \
                        fout, fam_out, stat_out, gpu = cls_gpu, multi = args.eval_multi)
        else: y_test_pred, neg_by_fam, family_to_idx = eval_classifier(args, classifier, cur_month_str, X_test_feat, y_test_binary, all_test_family, train_families, \
                        fout, fam_out, stat_out, gpu = cls_gpu, multi = args.eval_multi)
        ...
        if args.al == True and cur_month != end:
            ...
            if args.ood == True:
                ...
                sample_indices, sample_scores = selector.select_samples(args, \
                                                                ...
                                                                args.count, plot_step = EMBEDDING_PLOT_EVERY_N_STEPS , epoch = al_step_counter)
            else:
                raise ValueError('Unknown sampling method')
            
            scores_arr = np.asarray(sample_scores).ravel()
            for i in range(len(y_test_pred_accum)):
                try:
                    fam = all_test_family_accum[i]
                except IndexError:
                    fam = 'benign'
                pred_label = int(y_test_pred_accum[i])
                if args.classifier == 'gbdt':
                    pred_score = float(pred_scores_accum[i])
                else:
                    pred_score = float(pred_scores_accum[i][pred_label])
                test_unc_out.write('%s\t%d\t%d\t%d\t%s\t%d\t%s\t%.6f\t%.6f\n' % (
                    cur_month_str, i, test_offset + i, int(y_test_binary_accum[i]),
                    str(y_test_accum[i]), int(y_test_pred_accum[i]),
                    fam, float(scores_arr[i]), pred_score))
            test_unc_out.flush()
            ...
            for idx in sample_indices:
                ...
                if args.classifier == 'gbdt':
                    sample_out.write('%s\t%d\t%d\t%d\t%s\t%.4f\t%s\t%.4f\n' % \
                                (cur_month_str, cnt, idx, test_offset + idx, y_test_binary_accum[idx], pred_scores_accum[idx], fam_label, sample_scores[idx]))
                else:
                    sample_out.write('%s\t%d\t%d\t%d\t%s\t%.4f\t%s\t%.4f\n' % \
                                (cur_month_str, cnt, idx, test_offset + idx, y_test_binary_accum[idx], pred_scores_accum[idx][pred_label], fam_label, sample_scores[idx]))
                cnt += 1
            sample_out.flush()
            ...
            logging.info(f'X_train shape: {X_train.shape}')
            logging.info(f'X_train_buffer shape: {X_train_buffer.shape}')
            
            uncertain_indices = sample_indices[:args.index]
            certain_indices = sample_indices[args.index:]
            uncertain_true_y = y_test_accum[uncertain_indices]
            certain_pred_y = y_test_pred_accum[certain_indices]
            original_y = np.concatenate((uncertain_true_y, certain_pred_y), axis=0)
            
            uncertain_true_y_binary = y_test_binary_accum[uncertain_indices]
            if args.multi_class == True:
                certain_pred_y_binary = np.where(y_test_pred_accum[certain_indices] == 0, 0, 1)
            else:
                certain_pred_y_binary = y_test_pred_accum[certain_indices]
                
            y_train_binary_new = np.concatenate((uncertain_true_y_binary, certain_pred_y_binary), axis=0)
            y_train_binary = np.concatenate((y_train_binary, y_train_binary_new), axis=0)
            logging.info(f'y_train_binary shape: {y_train_binary.shape}')
            logging.info(f'y_train_binary_buffer shape: {y_train_binary_buffer.shape}')
            ...
            logging.info(f'y_train_buffer shape: {y_train_buffer.shape}')
            logging.info(f'y_test_accum[sample_indices] {y_test_accum[sample_indices]}')
            logging.info(f'new_y {new_y}')

            seen_families_before = set(all_train_family.flatten())
            all_train_family = np.concatenate((all_train_family, all_test_family_accum[sample_indices]), axis=0)

            new_fam_pos = [
                p for p in range(min(args.index, len(sample_indices)))
                if all_test_family_accum[sample_indices[p]] not in seen_families_before
                and y_test_binary_accum[sample_indices[p]] == 1
            ]
            new_fam_analyst_idx = [sample_indices[p] for p in new_fam_pos]

            logging.info(f'[{cur_month_str}] new_fam_pos: {new_fam_pos}')
            logging.info(f'[{cur_month_str}] new_fam_analyst_idx: {new_fam_analyst_idx}')
            for p in new_fam_pos:
                idx = sample_indices[p]
                logging.info(f'[{cur_month_str}] NEW FAM -> pos={p}, test_idx={idx}, '
                             f'family={all_test_family_accum[idx]}, '
                             f'y={new_y[p]}, y_bin={y_train_binary_new[p]}')

            logging.info(f'[{cur_month_str}] malware buffer BEFORE: {malware_X_buffer.shape[0]}')

            if len(new_fam_pos) > 0:
                malware_X_buffer = np.concatenate((malware_X_buffer, X_test_accum[new_fam_analyst_idx]), axis=0)
                malware_y_buffer = np.concatenate((malware_y_buffer, new_y[new_fam_pos]), axis=0)
                malware_y_binary_buffer = np.concatenate((malware_y_binary_buffer, y_train_binary_new[new_fam_pos]), axis=0)

            logging.info(f'[{cur_month_str}] malware buffer AFTER: {malware_X_buffer.shape[0]} '
                         f'(+{len(new_fam_pos)})')
            logging.info(f'[{cur_month_str}] malware buffer family dist: '
                         f'{Counter(malware_y_buffer.tolist()).most_common(5)} (top 5)')

            new_fam_pos_set = set(new_fam_pos)
            keep_pos = [p for p in range(len(sample_indices)) if p not in new_fam_pos_set]

            logging.info(f'[{cur_month_str}] sampled: total={len(sample_indices)}, '
                         f'moved_to_malware={len(new_fam_pos)}, kept={len(keep_pos)}')

            sampled_X = X_test_accum[sample_indices][keep_pos]
            sampled_y = new_y[keep_pos]
            sampled_y_binary = y_train_binary_new[keep_pos]

            X_train_buffer = np.concatenate((malware_X_buffer, sampled_X), axis=0)
            y_train_buffer = np.concatenate((malware_y_buffer, sampled_y), axis=0)
            y_train_binary_buffer = np.concatenate((malware_y_binary_buffer, sampled_y_binary), axis=0)

            logging.info(f'[{cur_month_str}] X_train_buffer: {X_train_buffer.shape} '
                         f'= malware({malware_X_buffer.shape[0]}) + sampled({sampled_X.shape[0]})')
            logging.info(f'[{cur_month_str}] y_train_buffer: {y_train_buffer.shape}')
            logging.info(f'[{cur_month_str}] y_train_binary_buffer: {y_train_binary_buffer.shape}')
            ...
            X_train_final = X_train_buffer
            y_train_final = y_train_buffer
            y_train_binary_final = y_train_binary_buffer
            upsample_values = None
            ...
            if args.encoder_retrain == True:
                ...
                if args.encoder != None and args.encoder != 'mlp':
                    ...
                    logging.info(f"X_train_final: {X_train_final.shape}")
                    logging.info(f"y_train_final: {y_train_final.shape}")
                    logging.info(f"X_train_final memory usage: {X_train_final.nbytes/1024**2:.2f} MB")
                    logging.info(f"y_train_final memory usage: {y_train_final.nbytes/1024**2:.2f} MB")
                    ...
                    total_training_time += (e2 - s2)    
                    
                    torch.cuda.synchronize()
                    round_peak_gpu_mem = torch.cuda.max_memory_allocated() / 1024**2
                    overall_peak_gpu_mem = max(overall_peak_gpu_mem, round_peak_gpu_mem)
                elif args.encoder == 'mlp':
                    s2 = time.time()
                    if args.cold_start == True:
                        if args.multi_class == True:
        ...
        prev_train_size = X_train.shape[0]
        logging.info(f'prev_train_size {prev_train_size}')
        # increment to next month
        cur_month += relativedelta(months=1)
    
    selector.select_samples(args, X_train, y_train, y_train_binary, \
                                                                X_test_accum, y_test_pred_accum, \
                                                                total_epochs, \
                                                                test_offset, \
                                                                fully_train_family, \
                                                                args.count, flag=True, epoch='final')
    logging.info(f'Total training time for encoder and classifier retraining: {total_training_time:.3f} seconds')
    logging.info(f'Peak GPU Memory Allocated during training: {overall_peak_gpu_mem:.2f} MB')
